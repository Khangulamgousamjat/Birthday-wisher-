...
class Particle {
  ...
  constructor(width: number, height: number, x?: number, y?: number, isClickBurst = false) {
...
class BurstParticle {
  ...
  constructor(x: number, y: number) {
    ...
    this.colors = ['rgba(255, 0, 150, 1)', 'rgba(255, 50, 200, 1)', 'rgba(255, 255, 255, 1)', 'rgba(200, 0, 100, 1)'];
...
export function PinkSparkles({ clickPosition }: PinkSparklesProps) {
  ...
  const particlesArrayRef = useRef<(Particle | BurstParticle)[]>([]);
  ...
  useEffect(() => {
    ...
    for (let i = 0; i < particleCount; i++) {
        particlesArrayRef.current.push(new Particle(canvas.width, canvas.height));
    }
  ...
  useEffect(() => {
    const canvas = canvasRef.current;
    if (clickPosition && particlesArrayRef.current && canvas) {
        // Spawn 40 particles at the click location
        for (let i = 0; i < 40; i++) {
            particlesArrayRef.current.push(new BurstParticle(clickPosition.x, clickPosition.y));
        }
    }
  }, [clickPosition]);
