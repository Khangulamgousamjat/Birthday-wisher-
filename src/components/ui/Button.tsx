  ...
  ({ className, variant = "primary", children, ...props }, ref) => {
    if (variant === "primary") {
      return (
        <motion.button
          ...
          whileTap={{ scale: 0.95 }}
          className={cn(
            "group relative dark:bg-neutral-800 bg-neutral-200 rounded-full p-px overflow-hidden min-w-[12rem] h-12 flex items-center justify-center transition-all duration-500",
            className
          )}
          {...props}
        >
          <span className="absolute inset-0 rounded-full overflow-hidden">
            <span className="inset-0 absolute pointer-events-none select-none">
              <span
                className="block -translate-x-1/2 -translate-y-1/3 size-24 blur-xl opacity-50"
          ...
          <span
            className="flex items-center justify-center gap-1 relative z-[1] dark:bg-neutral-950/90 bg-neutral-50/90 rounded-full py-2 px-4 w-full h-full"
          >
            <span
            ...
            <span
              className="bg-gradient-to-b ml-1.5 dark:from-white dark:to-white/50 from-neutral-950 to-neutral-950/50 bg-clip-text text-sm font-bold text-transparent group-hover:scale-105 transition transform-gpu"
            >
              {children}
            </span>
          </span>
    ...
    return (
      <motion.button
        ...
        whileTap={{ scale: 0.95 }}
        className={cn(
          "relative inline-flex items-center justify-center rounded-xl px-8 py-3 text-sm font-medium transition-all group overflow-hidden border border-white/10 glass text-white hover:bg-white/10",
          className
        )}
        {...props}
      >
